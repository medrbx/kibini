...
sub localisation {
    ...
    my $isItemnumber = 0 ;
    my $isLoc = 0 ;
    my $req = "SELECT reserve_id, itemnumber FROM stat_reserves2" ;
    my $dbh = GetDbh() ;
    my $sth = $dbh->prepare($req);
    $sth->execute();
    while ( my @row = $sth->fetchrow_array ) {
        my ($reserve_id, $itemnumber) = @row ;
        if ( defined $itemnumber ) {
            $isItemnumber++ ;
            my $espace = localisation( $itemnumber ) ;
            if ( defined $espace ) {
                my $req = <<SQL ;
UPDATE statdb.stat_reserves2 s 
SET espace = ?
...
SQL
    ...
    $sth->finish();
    $dbh->disconnect() ;
    print "$reserve, $isItemnumber, $isLoc\n" ;
