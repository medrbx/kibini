...
sub exemplaires {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2019_exemplaires_$nom_fichier.csv") ;
...
SELECT
    ...
    COUNT(IF(i.dateaccessioned > ('2020-01-01' - INTERVAL 1 YEAR), i.itemnumber, NULL)) AS 'nb_exemplaires_créés_dans_annee',
	COUNT(IF(i.notforloan NOT IN (-2, -1, 2) AND (i.datelastborrowed <= ('2020-01-01' - INTERVAL 1 YEAR) OR i.datelastborrowed IS NULL), i.itemnumber, NULL)) AS 'nb_exemplaires_empruntables_pas_empruntés_1_an',
    COUNT(IF(i.notforloan NOT IN (-2, -1, 2) AND (i.datelastborrowed <= ('2020-01-01' - INTERVAL 3 YEAR) OR i.datelastborrowed IS NULL), i.itemnumber, NULL)) AS 'nb_exemplaires_empruntables_pas_empruntés_3_ans',
    COUNT(IF(i.onloan IS NOT NULL OR i.itemlost = 1, i.itemnumber, NULL)) AS 'nb_exemplaires_en_pret'
FROM koha2019.items i
JOIN koha2019.biblioitems bi ON bi.biblionumber = i.biblionumber
...
sub prets {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2019_prets_$nom_fichier.csv") ;
    my @column_names = qw( collection_code support nb_prets_2017 nb_prets_2017_exemplaires_distincts nb_prets_2017_emprunteurs_distincts nb_prets_2018 nb_prets_2018_exemplaires_distincts nb_prets_2018_emprunteurs_distincts nb_prets_2019 nb_prets_2019_exemplaires_distincts nb_prets_2019_emprunteurs_distincts ) ;
    $csv->print ($fd, \@column_names) ;
...
SELECT
	...
	COUNT(IF(YEAR(i.issuedate) = 2018, i.itemnumber, NULL)) AS 'nb_prets_2018',
	COUNT(DISTINCT(IF(YEAR(i.issuedate) = 2018, i.itemnumber, NULL))) AS 'nb_prets_2018_exemplaires_distincts',
	COUNT(DISTINCT(IF(YEAR(i.issuedate) = 2018, i.borrowernumber, NULL))) AS 'nb_prets_2018_emprunteurs_distincts',
...
sub eliminations {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2019_eliminations_$nom_fichier.csv") ;
