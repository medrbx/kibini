...
sub exemplaires {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2018_exemplaires_v2_zd_$nom_fichier.csv") ;
...
SELECT
    ...
    COUNT(IF(i.dateaccessioned > ('2019-01-01' - INTERVAL 1 YEAR), i.itemnumber, NULL)) AS 'nb_exemplaires_créés_dans_annee',
	COUNT(IF(i.notforloan NOT IN (-2, -1, 2) AND (i.datelastborrowed <= ('2019-01-01' - INTERVAL 1 YEAR) OR i.datelastborrowed IS NULL), i.itemnumber, NULL)) AS 'nb_exemplaires_empruntables_pas_empruntés_1_an',
    COUNT(IF(i.notforloan NOT IN (-2, -1, 2) AND (i.datelastborrowed <= ('2019-01-01' - INTERVAL 3 YEAR) OR i.datelastborrowed IS NULL), i.itemnumber, NULL)) AS 'nb_exemplaires_empruntables_pas_empruntés_3_ans',
...
sub prets {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2018_prets_zd_$nom_fichier.csv") ;
    my @column_names = qw( collection_code support nb_prets_2016 nb_prets_2016_exemplaires_distincts nb_prets_2016_emprunteurs_distincts nb_prets_2017 nb_prets_2017_exemplaires_distincts nb_prets_2017_emprunteurs_distincts nb_prets_2018 nb_prets_2018_exemplaires_distincts nb_prets_2018_emprunteurs_distincts) ;
    $csv->print ($fd, \@column_names) ;
...
sub eliminations {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2018_eliminations_zd_$nom_fichier.csv") ;
