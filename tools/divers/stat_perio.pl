...
my @column_out = ( 'Thématique', 'Titre', 'Numéro notice bib', '1er destination', '2e destination', 'empruntable ?', 'infos', 'Périodicité', 'prix de l\'abt', 'Nb prêts', 'Emprunteurs', 'Prêts ex dist', 'Ex' ) ;
...
while (my $row = $csv_in->getline_hr ($fd_in)) {
    ...
    my @row_out = ( $row->{thematique}, $row->{titre}, $row->{biblionumber}, $row->{dest1}, $row->{dest2}, $row->{empruntable}, $row->{infos}, $row->{periodicite}, $row->{prix}, $row->{nb_issues}, $row->{nb_emprunteurs_dist}, $row->{nb_items_dist_emp}, $row->{nb_items} ) ;
...
sub GetNbIssues {
    ...
    my $req = "SELECT COUNT(issue_id) FROM statdb.stat_issues  WHERE biblionumber = ? AND DATE(issuedate) BETWEEN '2016-09-01' AND '2017-08-31'" ;
...
sub GetNbDistBorrowers {
    ...
    my $req = "SELECT COUNT(DISTINCT borrowernumber) FROM statdb.stat_issues  WHERE biblionumber = ? AND DATE(issuedate) BETWEEN '2016-09-01' AND '2017-08-31'" ;
    my $sth = $dbh->prepare($req) ;
    $sth->execute($biblionumber) ;
    return $sth->fetchrow_array() ;
    $sth->finish() ;     
...
sub GetNbDistItemsLoaned {
    ...
    my $req = "SELECT COUNT(DISTINCT itemnumber) FROM statdb.stat_issues  WHERE biblionumber = ? AND DATE(issuedate) BETWEEN '2016-09-01' AND '2017-08-31'" ;
    my $sth = $dbh->prepare($req) ;
    $sth->execute($biblionumber) ;
    return $sth->fetchrow_array() ;
    $sth->finish() ;     
...
sub GetNbItems {
    ...
    foreach my $table (@tables) {
        my $req = "SELECT COUNT(itemnumber) FROM koha_prod.$table  WHERE biblionumber = ? AND DATE(dateaccessioned) BETWEEN '2016-09-01' AND '2017-08-31'" ;
        my $sth = $dbh->prepare($req) ;
        $sth->execute($biblionumber) ;
