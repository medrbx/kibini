...
print Dumper($surprises) ;
my $type ;
#while( my ($biblionumber, $d) = each(%$surprises) ) {
#    my @dates = @$d ;
#	if ($biblionumber=~m/^Stat/ ) {
#		$type = $biblionumber ;
#		print "$type\n" ;
#	}
    #if ( $biblionumber=~m/^\d+$/ ) {
	...
	#	$count->{type} = $type ;
    #   print "$count->{type},$count->{biblionumber},$count->{surprise_datecreated},$count->{biblio_datecreated},$count->{before},$count->{after}\n" ;
    #}
#}
...
sub GetSurprisesStat {
    ...
    foreach my $res (@result) {
		if ( $result =~s/^Stat// ) {
			...
			if ( $surprises->{$type}->{$biblionumber} ) {
				my $dates = $surprises->{$type}->{$biblionumber} ;
				@dates = @$dates ;
				push @dates, $date ;
				my @dates = sort(@dates) ; # on classe les dates par ordre croissant
				$surprises->{$type}->{$biblionumber} = \@dates ;
			} else {
				push @dates, $date ;
				$surprises->{$type}->{$biblionumber} = \@dates ;
			}
