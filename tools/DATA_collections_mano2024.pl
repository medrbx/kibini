...
sub exemplaires {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2024_exemplaires_$nom_fichier.csv") ;
...
FROM koha2024.items i
JOIN koha2024.biblioitems bi ON bi.biblionumber = i.biblionumber
...
sub prets {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2024_prets_$nom_fichier.csv") ;
    my @column_names = qw( collection_code support nb_prets_2022 nb_prets_2022_exemplaires_distincts nb_prets_2022_emprunteurs_distincts nb_prets_2023 nb_prets_2023_exemplaires_distincts nb_prets_2023_emprunteurs_distincts nb_prets_2024 nb_prets_2024_exemplaires_distincts nb_prets_2024_emprunteurs_distincts) ;
    $csv->print ($fd, \@column_names) ;
...
SELECT
    i.ccode,
	i.itemtype,
	COUNT(IF(YEAR(i.issuedate) = 2022, i.itemnumber, NULL)) AS 'nb_prets_2022',
	COUNT(DISTINCT(IF(YEAR(i.issuedate) = 2022, i.itemnumber, NULL))) AS 'nb_prets_2022_exemplaires_distincts',
	COUNT(DISTINCT(IF(YEAR(i.issuedate) = 2022, i.borrowernumber, NULL))) AS 'nb_prets_2022_emprunteurs_distincts',
...
sub eliminations {
    ...
    open(my $fd,">:encoding(utf8)","../data/collections/2024_eliminations_$nom_fichier.csv") ;
